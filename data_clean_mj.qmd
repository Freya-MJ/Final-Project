...
在这个项目中我们使用了来自多个来源的数据，我们选取的数据主要时间是从2015-2022年各州的不同指标，其中包括poverty rate，population，median income，high school graduation numbers，land area， water area， total area，unemployment rate。获取这些数据的方法主要包括网络直接下载，以及API获取。由于不同来源的数据有不同的格式，在清洗的步骤中，我们通过使用pivot_longer，以及columnnames等命令，将数据转换成同样的格式，方便我们对于数据进行合并以及清洗。在初步观察data之后，删除掉了缺少较多数据的变量，一些较少的缺失值我们使用0去代替
...
write.csv(data_clean, file = "data_clean_mj.csv")
```
